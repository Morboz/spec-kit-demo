...
from src.services.ai_strategy import RandomStrategy, Move
...
def test_ai_player_calculate_move_interface():
    ...
    assert hasattr(ai_player, 'calculate_move'), "AIPlayer should have calculate_move method"
...
def test_different_strategies_produce_different_moves():
    ...
    from src.services.ai_strategy import RandomStrategy, CornerStrategy
    ...
    random_ai = AIPlayer(player_id=1, strategy=RandomStrategy(), color="blue", name="Random")
    corner_ai = AIPlayer(player_id=2, strategy=CornerStrategy(), color="red", name="Corner")
