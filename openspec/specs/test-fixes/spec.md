...
## What Changes
- 修复集成测试中GameState API错误使用
- 更新测试以使用正确的GameState初始化和turn推进方法
- 修复变量作用域问题

## Why
集成测试失败是因为测试代码使用了不存在的GameState方法：
1. `GameState.initialize(n_players)` - 方法不存在
2. `GameState.advance_turn()` - 方法不存在，实际是 `next_turn()`
3. 变量作用域错误

这不是源码bug，而是测试设计错误。GameState API本身是正确的。
## Requirements
